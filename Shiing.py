...
class SkierClass(pygame.sprite.Sprite):
    ...
    def turn(self, direction):
        ...
        speed = [self.angle * 3, 10 + abs(baseSpeed) - abs(self.angle) * 2] #Increased base speed from 6 to 10
...
enchant = 0
furnace = 0
for i in range(0,23):#MOD
    randomee = random.randint(0,23)
    pygame.mixer_music.queue(backgroundMusicNames[randomee])#MOD
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:  # check for key presses
            if event.key == pygame.K_a:  # A turns left
                ...
                speed = steve.turn(1)
            elif event.key == pygame.K_w:#MOD
                baseSpeed += 2
            elif event.key == pygame.K_s:#MOD
                baseSpeed -= 2
